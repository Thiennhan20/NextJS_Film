...
const CATEGORY = {
  id: 'horror',
  titleKey: 'horrorMovies',
  defaultTitle: 'Kinh Dị Giật Gân',
  endpoint: '/discover/movie?with_genres=27&sort_by=popularity.desc',
  type: 'movie' as const,
  color: 'from-gray-700 to-black',
};

export default function HorrorFrame() {
  ...
  const fetchData = useCallback(async () => {
    const [moviesRes, tvRes] = await Promise.all([
      axios.get(`/api/tmdb-proxy?endpoint=${encodeURIComponent('/discover/movie?with_genres=27&sort_by=popularity.desc')}`),
      axios.get(`/api/tmdb-proxy?endpoint=${encodeURIComponent('/discover/tv?with_genres=9648&sort_by=popularity.desc')}`)
    ]);
    ...
    <div className="mb-10 sm:mb-12">
      <div className="flex items-center gap-3 mb-4 px-3">
        ...
        <h3 className="flex items-center gap-2 text-xl sm:text-2xl font-bold text-white tracking-wide">
          ...
          <motion.span
            aria-hidden="true"
            animate={prefersReducedMotion ? undefined : { y: [0, -3, 0], opacity: [1, 0.78, 1] }}
            transition={{ duration: 2.2, repeat: Infinity, ease: 'easeInOut' }}
          >
            <GiGhost className="h-5 w-5 text-slate-200 sm:h-6 sm:w-6" />
          </motion.span>
        </h3>
      </div>
