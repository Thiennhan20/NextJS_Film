...
const CATEGORY = {
  ...
  defaultTitle: 'Hành Động Nghẹt Thở',
...
export default function ActionFrame() {
  ...
  const fetchData = useCallback(async () => {
    const [moviesRes, tvRes] = await Promise.all([
      axios.get(`/api/tmdb-proxy?endpoint=${encodeURIComponent('/discover/movie?with_genres=28&sort_by=popularity.desc')}`),
      axios.get(`/api/tmdb-proxy?endpoint=${encodeURIComponent('/discover/tv?with_genres=10759&sort_by=popularity.desc')}`)
    ]);
