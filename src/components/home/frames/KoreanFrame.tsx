...
const CATEGORY = {
  id: 'korean',
  titleKey: 'koreanMovies',
  defaultTitle: 'Phim Hàn',
  color: 'from-pink-500 to-rose-500',
};

export default function KoreanFrame() {
  ...
  const fetchData = useCallback(async () => {
    const [moviesRes, tvRes] = await Promise.all([
      axios.get(`/api/tmdb-proxy?endpoint=${encodeURIComponent('/discover/movie?with_original_language=ko&sort_by=popularity.desc')}`),
      axios.get(`/api/tmdb-proxy?endpoint=${encodeURIComponent('/discover/tv?with_original_language=ko&sort_by=popularity.desc')}`)
    ]);
