  ...
  return {
    ...
    seasons: (data.seasons || [])
      ...
      .map((season) => ({
        ...
        voteAverage: Number(season.vote_average || 0),
      })),
  }
}
...
export async function fetchSeasonEpisodes(tvId: number, season: number) {
  const response = await axios.get<{ episodes?: MediaEpisode[]; air_date?: string; vote_average?: number }>(
    `/api/tmdb-proxy?endpoint=/tv/${tvId}/season/${season}`
  )
  return {
    episodes: response.data.episodes || [],
    airDate: response.data.air_date || '',
    voteAverage: Number(response.data.vote_average || 0),
  }
}
